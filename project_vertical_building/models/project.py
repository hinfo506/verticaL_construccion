...
class ProjectProject(models.Model):
    ...
    stock_location_id = fields.Many2one(
        comodel_name="stock.location", string="Ubicación Stock", required=False
    )
    supervisor_id = fields.Many2one(
        comodel_name="res.users", string="Supervisor de almacén", required=False
    )
    ...
    invoice_address_id = fields.Many2one(
        comodel_name="res.partner", string="Dirección de Facturación", required=False
    )
    analitic_id = fields.Many2one(
    ...
    # FASES DEL PROYECTO
    vertical_stage_ids = fields.One2many(
        comodel_name="vertical.stage",
        inverse_name="project_id",
        string="vertical_stage_ids",
        required=False,
    )
    ...
    def action_view_fase(self):
        ...
        return {
            ...
            "context": dict(
                ...
                searchpanel_project_building_name=self.name,
                project_id=self.id,
            ),
        }
    ...
    fase_kanban_count = fields.Integer(
        string="FasePrincipal_kanban_count",
        compute="_compute_fase_count",
        required=False,
    )
