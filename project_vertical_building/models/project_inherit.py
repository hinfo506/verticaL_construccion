...
class ProyectosInherit(models.Model):
    ...
    # FASES DEL PROYECTO
    fase_id = fields.One2many(comodel_name='fase.fase',inverse_name='project_id', string='Fase_id', required=False)
    item_ids = fields.One2many(comodel_name='item.item', inverse_name='project_id', string='Item_ids', required=False)
    ...
    def _compute_item_count(self):
        task_data = self.env['item.item'].read_group(
            [('project_id', 'in', self.ids)],
            ['project_id', 'project_id:count'], ['project_id'])
        result_with_subtasks = defaultdict(int)
        for data in task_data:
