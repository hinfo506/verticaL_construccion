...
class ProyectosInherit(models.Model):
    ...
    warehouse = fields.Many2one(
        comodel_name="stock.warehouse", string="Almacén", required=False
    )
    ...
    # FASES DEL PROYECTO
    fase_principal_ids = fields.One2many(
        comodel_name="fase.principal",
        inverse_name="project_id",
        string="Fase_principal_ids",
        required=False,
    )
    capitulos_id = fields.One2many(
        comodel_name="capitulo.capitulo",
        inverse_name="project_id",
        string="Capitulos_id",
        required=False,
    )
    ...
    fase_principal_kanban_count = fields.Integer(
        string="FasePrincipal_kanban_count",
        compute="_compute_faseprincipal_count",
        required=False,
    )
    capitulos_kanban_count = fields.Integer(
        string="Capitulos_kanban_count",
        compute="_compute_capitulo_count",
        required=False,
    )
    subcapitulos_kanban_count = fields.Integer(
        string="Subcapitulos_kanban_count",
        compute="_compute_subcapitulo_count",
        required=False,
    )
    partidas_kanban_count = fields.Integer(
        string="Partidas_kanban_count",
        compute="_compute_partidas_count",
        required=False,
    )
    item_kanban_count = fields.Integer(
        string="Item_kanban_count", compute="_compute_item_count", required=False
    )
    ...
    def met_partidas(self):
        return {
            ...
            "view_mode": "tree,form",
            "domain": [
                ("id", "in", self.capitulos_id.subcapitulo_ids.partidas_ids.ids)
            ],
            "context": dict(self._context, default_project_id=self.id),
        }
        ...
        return {
            ...
            "domain": [
                (
                    "id",
                    "in",
                    self.capitulos_id.subcapitulo_ids.partidas_ids.item_capitulo_ids.ids,
                )
            ],
            "context": dict(self._context, default_project_id=self.id),
        }
    ...
    def wizard_cambio_precio(self):
        return {
            ...
            "context": {
                ...
                "default_info": "LOS PRECIOS SERAN CAMBIADOS A PARTIR DE </br><strong>"
                + str(self.name)
                + " :</strong>",
            },
            "type": "ir.actions.act_window",
        ...
        for proyecto in copia_proyecto:
            for faseprincipal_id in proyecto.fase_principal_ids:
                ...
                for capitulo_id in faseprincipal_id.capitulos_ids:
                    capitulo_id.subcapitulo_ids.write(
                    ...
                    for subcapitulo_id in capitulo_id.subcapitulo_ids:
                        subcapitulo_id.partidas_ids.write(
                            {
                                "project_id": copia_proyecto.id,
                                "capitulo_id": capitulo_id.id,
                            }
                        )
                        for partidas_id in subcapitulo_id.partidas_ids:
                            ...
                            partidas_id.volumetria_ids.write(
                                {
                                    "project_id": proyecto.id,
                                    "capitulo_id": capitulo_id.id,
                                    "subcapitulo_id": subcapitulo_id.id,
    ...
    def write(self, values):
        ...
        if values.get("stage_id"):
            ...
            if not stage.is_prevision and self.total_prevision == 0:
                ...
                # Fase Principal
                fase_ids = self.env["fase.principal"].search(
                    [("project_id", "=", self.id)]
                )
                ...
                # Capitulo
                capitulo_ids = self.env["capitulo.capitulo"].search(
                    [("project_id", "=", self.id)]
                )
                ...
                # Subcapitulo
                subcapitulos_ids = self.env["sub.capitulo"].search(
                    [("project_id", "=", self.id)]
                )
                ...
                # Partida
                partidas_ids = self.env["partidas.partidas"].search(
                    [("project_id", "=", self.id)]
                )
                ...
                # Item
                items_ids = self.env["item.capitulo"].search(
                    [("project_id", "=", self.id)]
                )
