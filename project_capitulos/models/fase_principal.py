...
class FaseInicial(models.Model):
    ...
    capitulos_ids = fields.One2many(
        comodel_name="capitulo.capitulo",
        inverse_name="fase_principal_id",
        string="Capitulos_id",
        required=False,
    )
    item_ids = fields.One2many(
        comodel_name="item.capitulo",
        inverse_name="faseprincipal_id",
        string="Item_ids",
        required=False,
    )

    etapa_id = fields.Many2one(
        comodel_name="stage.stage", string="Etapa", required=False
    )
    ...
    location = fields.Many2one(
        comodel_name="stock.location", string="Ubicación en Almacén", required=False
    )

    @api.onchange("number", "project_id")
    def _onchange_join_number_faseprincipal(self):
        self.numero_fase_principal = (
            str(self.project_id.numero_proyecto) + "." + str(self.number)
        )
    ...
    def get_count_capitulos(self):
        for r in self:
            count = self.env["capitulo.capitulo"].search_count(
                [("fase_principal_id", "=", self.id)]
            )
