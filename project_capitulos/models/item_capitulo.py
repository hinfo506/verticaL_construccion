...
class ItemCapitulo(models.Model):
    ...
    product_qty = fields.Float(
        string="Cantidad Planificada", copy=False, digits=(12, 2)
    )
    ...
    faseprincipal_id = fields.Many2one(
        "fase.principal", string="Fase Principal", ondelete="cascade"
    )
    capitulo_id = fields.Many2one(
        "capitulo.capitulo", string="Capitulo", ondelete="cascade"
    )
    subcapitulo_id = fields.Many2one(
        "sub.capitulo", string="Subcapitulo", ondelete="cascade"
    )
    partidas_id = fields.Many2one(
        "partidas.partidas", string="Partidas", ondelete="cascade"
    )
    ...
    # Campos Sumatorios
    item_volumetria_ids = fields.One2many(
        comodel_name="item.volumetria",
        inverse_name="itemcapitulo_id",
        string="Item Volumetria",
        required=False,
    )
    itemvolumetria_count = fields.Integer(
        string="Itemvolumetria_count", required=False, compute="get_itemvolu_count"
    )
    ...
    tipo_descuento = fields.Selection(
        string="Tipo descuento Proveedor",
        selection=[
            ("cantidad", "cantidad"),
            ("porciento", "porciento"),
    ...
    def get_itemvolu_count(self):
        for r in self:
            r.itemvolumetria_count = self.env["item.volumetria"].search_count(
                [("itemcapitulo_id", "=", self.id)]
            )
