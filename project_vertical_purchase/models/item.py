...
class VerticalItem(models.Model):
    ...
    def purchase_from_item(self):
        ...
        )  # tomo solo los items que son de tipo material
        ...
        for item in items:
            if item.job_type == "material":
                ...
                for seller in item.product_id.seller_ids.filtered(
                    ...
                    if seller.min_qty <= item.product_qty:
                        ...
                        name = product_lang.with_context(
                            seller_id=seller.id
                        ).display_name
                ...
                current_po = po_obj.create(
                    {
                        ...
                        "stage_id": items[
                            0
                        ].vertical_stage_id.id,  # Para que quiero el stage?
                    }
                )
                purchase_data[vendor_key][
                    "purchase_order"
                ] = current_po.id  # Actualizo mi dict
